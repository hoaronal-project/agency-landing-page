...
function load_quick_job_view(emp_id, comp_name){
	...
	$.getJSON(baseUrl+'admin/posted_jobs/latest_job_by_company/'+emp_id, function(data) {
			...
			if(data.err){
				...
				$("#job_desc").html(data.job_description.substr(0, 150)+'...');
			}
		});
}
...
//====== Remaining Days
$('#remaining_active_days').bind('keyup blur',function(){ 
    $(this).val( $(this).val().replace(/[^0-9]/g,'') ); 
});
 
function load_remaining_days_popup(job_id){
		$('#modal_remaining_days').modal('show');
		$('#emsg').html('');
		$("#jid").val(job_id);
		$.getJSON(baseUrl+'admin/posted_jobs/job_by_id/'+job_id, function(data) {
				$("#remaining_active_days").val(data.remaining_active_days);
			});
}

$("#submit_days").click(function(){
	var jid = $("#jid").val();
	var remaining_active_days = $("#remaining_active_days").val();
	
	if(jid==''){
		$('#emsg').html('<span class="label label-warning">Oops! Somehow job ID is missing, please refresh your page and try again.</span>');
		return false;
	}
	
	if(remaining_active_days==''){
		$('#emsg').html('<span class="label label-danger">Please provide remaining days.</span>');
		return false;
	}
	
	 $.ajax({
		  type: "POST",
		  url: baseUrl+"admin/posted_jobs/update_remaining_days",
		  data: { jid: jid, remaining_active_days: remaining_active_days }
			  })
				.done(function( msg ) {
					//obj = jQuery.parseJSON(data);
					if(msg=='done'){
						$('#emsg').html('<span class="label label-success">Updated Successfully!</span>');
						$('#modal_remaining_days').modal('toggle');
						$('#days_'+jid).html(remaining_active_days);
						//$('#days_'+jid).toggle( "highlight" );
						 closest_div = $('#days_'+jid).closest('tr');
...
function extend_days(jid, remaining_active_days){
	$.ajax({
		  type: "POST",
		  url: baseUrl+"admin/posted_jobs/extend_remaining_days",
		  data: { jid: jid, remaining_active_days: remaining_active_days }
			  })
				.done(function( data ) {
					obj = jQuery.parseJSON(data);
					if(obj.msg=='done'){
						$('#days_'+jid).html(obj.new_remaining_days);
						 closest_div = $('#days_'+jid).closest('tr');
						 closest_div.effect( "highlight", {color:"#FFFF66"}, 1000 );
					}
					else{
...
//Packages Section
function validate_edit_packages_form(){
		if($('#edit_package_name').val()==''){
			alert('Please provide package name.');
			return false;	
		}
		
		if($('#edit_package_days').val()==''){
			alert('Please provide package days.');
			return false;	
		}
		
		if($('#edit_package_price').val()==''){
			alert('Please provide package price.');
			return false;	
		}
}

function load_package_edit_form(id){
	
	$.getJSON(baseUrl+'admin/packages/get_record_by_id/'+id, function(data) {
			$('#edit_package_name').val(data.package_name);
			$('#edit_package_days').val(data.package_days);
			$('#edit_package_price').val(data.package_price);
			$('#package_id').val(data.ID);
			$('#edit_package_form').modal('show');
		});	
}

function load_package_add_form(id){
	$('#package_name').val('');
	$('#package_days').val('');
	$('#package_price').val('');
	$('#add_package_form').modal('show');
}

function delete_package(id){
	var myurl = baseUrl+'admin/packages/delete/'+id;
	var is_confirm = confirm("Are you sure you want to delete this package?");
	if(is_confirm){
		  $.get(myurl, function (sts) {
			  if(sts=='done')
				  $("#row_"+id).fadeOut();
...
function update_package_status(id){
	var current_status = $("#sts_"+id+" span").html();
	var myurl = baseUrl+'admin/packages/status/'+id+'/'+current_status;
	$.get(myurl, function (sts) {
		var class_label = 'success';
		if(sts!='active')
			var class_label = 'danger';
...
//Ends Packages Section


//CV Remaining Days Starts
$('#remaining_cv_active_days').bind('keyup blur',function(){ 
    $(this).val( $(this).val().replace(/[^0-9]/g,'') ); 
});
 
function load_cv_remaining_days_popup(cid){
		$('#modal_cv_remaining_days').modal('show');
		$('#emsg').html('');
		$("#cid").val(cid);
		$.getJSON(baseUrl+'admin/job_seekers/consultant_by_id/'+cid, function(data) {
				$("#remaining_cv_active_days").val(data);
			});
}

$("#submit_cv_days").click(function(){
	var cid = $("#cid").val();
	var remaining_cv_active_days = $("#remaining_cv_active_days").val();
	
	if(cid==''){
		$('#emsg').html('<span class="label label-warning">Oops! Somehow Candidate ID is missing, please refresh your page and try again.</span>');
		return false;
	}
	
	if(remaining_cv_active_days==''){
		$('#emsg').html('<span class="label label-danger">Please provide remaining days.</span>');
		return false;
	}
	 ...
	 $.ajax({
		  type: "POST",
		  url: baseUrl+"admin/job_seekers/update_cv_remaining_days",
		  data: { cid: cid, remaining_cv_active_days: remaining_cv_active_days }
			  })
				.done(function( msg ) {
					//obj = jQuery.parseJSON(data);
					if(msg=='done'){
						$('#emsg').html('<span class="label label-success">Updated Successfully!</span>');
						$('#modal_cv_remaining_days').modal('toggle');
						$('#days_'+cid).html(remaining_cv_active_days);
						//$('#days_'+jid).toggle( "highlight" );
						 closest_div = $('#days_'+cid).closest('tr');
						 closest_div.effect( "highlight", {color:"#FFFF66"}, 1000 );
					}
					else{
						$('#emsg').html('<span class="label label-danger">'+msg+'</span>');
...
function extend_cv_days(cid, remaining_cv_active_days){
	$.ajax({
		  type: "POST",
		  url: baseUrl+"admin/job_seekers/extend_cv_remaining_days",
		  data: { cid: cid, remaining_cv_active_days: remaining_cv_active_days }
			  })
				.done(function( data ) {
					obj = jQuery.parseJSON(data);
					if(obj.msg=='done'){
						$('#days_'+cid).html(obj.new_remaining_cv_days);
						 closest_div = $('#days_'+cid).closest('tr');
						 closest_div.effect( "highlight", {color:"#FFFF66"}, 1000 );
					}
					else{
...
//CV Remaining Days Ends


//Employer Remaining Days Starts
$('#subscription_last_date').bind('keyup blur',function(){ 
    $(this).val( $(this).val().replace(/[^0-9]/g,'') ); 
});
 
function load_emp_remaining_days_popup(eid){
		$('#modal_emp_remaining_days').modal('show');
		$('#emsg').html('');
		$("#eid").val(eid);
		$.getJSON(baseUrl+'admin/employers/employer_by_id/'+eid, function(data) {
				$("#subscription_last_date").val(data);
			});
}

$("#submit_emp_days").click(function(){
	var eid = $("#eid").val();
	var subscription_last_date = $("#subscription_last_date").val();
	
	if(eid==''){
		$('#emsg').html('<span class="label label-warning">Oops! Somehow Employer ID is missing, please refresh your page and try again.</span>');
		return false;
	}
	...
	if(subscription_last_date==''){
	 ...
	 $.ajax({
		  type: "POST",
		  url: baseUrl+"admin/employers/update_emp_remaining_days",
		  data: { eid: eid, subscription_last_date: subscription_last_date }
			  })
				.done(function( msg ) {
					//obj = jQuery.parseJSON(data);
					if(msg=='done'){
						$('#emsg').html('<span class="label label-success">Updated Successfully!</span>');
						$('#modal_emp_remaining_days').modal('toggle');
						$('#days_'+eid).html(subscription_last_date);
						//$('#days_'+jid).toggle( "highlight" );
						 closest_div = $('#days_'+eid).closest('tr');
						 closest_div.effect( "highlight", {color:"#FFFF66"}, 1000 );
					}
					else{
						$('#emsg').html('<span class="label label-danger">'+msg+'</span>');
...
function extend_emp_days(eid, subscription_last_date){
	$.ajax({
		  type: "POST",
		  url: baseUrl+"admin/employers/extend_emp_remaining_days",
		  data: { eid: eid, subscription_last_date: subscription_last_date }
			  })
				.done(function( data ) {
					obj = jQuery.parseJSON(data);
					if(obj.msg=='done'){
						$('#days_'+eid).html(obj.new_subscription_last_date);
						 closest_div = $('#days_'+eid).closest('tr');
						 closest_div.effect( "highlight", {color:"#FFFF66"}, 1000 );
					}
					else{
...
//Employer Remaining Days Ends


//Employer Remaining Available Posts Starts
$('#remaining_available_posts').bind('keyup blur',function(){ 
    $(this).val( $(this).val().replace(/[^0-9]/g,'') ); 
});
 
function load_emp_remaining_posts(eid){
		$('#modal_emp_remaining_posts').modal('show');
		$('#emsg2').html('');
		$("#eid2").val(eid);
		$('#remaining_available_posts').val('');
		$.getJSON(baseUrl+'admin/employers/employer_available_posts_by_id/'+eid, function(data) {
				$("#remaining_available_posts").val(data);
			});
}

$("#submit_emp_posts").click(function(){
	var eid = $("#eid2").val();
	var remaining_available_posts = $("#remaining_available_posts").val();
	
	if(eid==''){
		$('#emsg2').html('<span class="label label-warning">Oops! Somehow Employer ID is missing, please refresh your page and try again.</span>');
		return false;
	}
	
	if(remaining_available_posts==''){
		$('#emsg2').html('<span class="label label-danger">Please provide available posts.</span>');
		return false;
	}
	
	 $.ajax({
		  type: "POST",
		  url: baseUrl+"admin/employers/update_emp_remaining_posts",
		  data: { eid2: eid, remaining_available_posts: remaining_available_posts }
			  })
				.done(function( msg ) {
					//obj = jQuery.parseJSON(data);
					if(msg=='done'){
						$('#emsg2').html('<span class="label label-success">Updated Successfully!</span>');
						$('#modal_emp_remaining_posts').modal('toggle');
						$('#remaining_available_posts').val('');
						$('#posts_'+eid).html(remaining_available_posts);
						 closest_div = $('#posts_'+eid).closest('tr');
						 closest_div.effect( "highlight", {color:"#FFFF66"}, 1000 );
					}
					else{
						$('#emsg2').html('<span class="label label-danger">'+msg+'</span>');
					}
		});
});

//Employer Available Posts Ends
