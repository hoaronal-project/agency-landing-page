...
class MY_Form_validation extends CI_Form_validation {
	...
	public function validate_ml_spam($captcha) {
        $CI = & get_instance();
        $CI->form_validation->set_message('validate_ml_spam', 'Invalid Verification Code Code Entered.');
	...
	{
		list($table, $field, $except_id)=explode('.', $field);
		//$query = $this->CI->db->limit(1)->get_where($table, array($field => $str));
		
		$this->CI->db->where($field, $str);
		$this->CI->db->where('ID !=', $except_id);
		$this->CI->db->from($table);
		//echo $this->CI->db->count_all_results(); exit;
		$this->CI->form_validation->set_message('is_unique_except', 'This email address is already taken.');
