...
{	
	function count_days( $a, $b )
	{
		...
		$startTimeStamp = date_create($a);
		$endTimeStamp = date_create($b);
		$diff=date_diff($startTimeStamp,$endTimeStamp);
		$days = ($diff->format("%r%a")<1?'0':$diff->format("%a"));
		return $days;
	}
}
...
{
	function is_active_like($current_page, $page)
	{
...
if ( ! function_exists('active_link'))
{
	function active_link($link){
		...
		if($CI->uri->segment(1)==$link)
				$data='class="current-menu-item"';
			else
				$data='class="inactive"';
		return $data;
	}
}
if ( ! function_exists('active_link_jobseeker'))
{
	function active_link_jobseeker($link){
		$CI =& get_instance();
		$data = '';
